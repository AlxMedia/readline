=== Readline ===
Contributors: alxmedia
Requires at least: 5.0
Tested up to: 6.0
License: GPLv3
License URI: http://www.gnu.org/licenses/gpl-3.0
Tags: blog, news, entertainment, one-column, two-columns, right-sidebar, left-sidebar, custom-colors, custom-menu, featured-images, flexible-header, full-width-template, post-formats, sticky-post, theme-options, threaded-comments, translation-ready, custom-logo, custom-header, custom-background, footer-widgets

== Description ==

Readline is a flexible magazine and personal blog theme with plenty of options. Optimized for all devices, this theme will stand out with a unique posts page that will inspire people to read. It includes a featured posts carousel. Set the theme to any accent color you wish. Use a sidebar to the left or right, or disable it entirely - it is all up to you. Demo: http://demo.alx.media/x/?theme=Readline

== Installation ==

1. In your admin panel, go to Appearance -> Themes and click the 'Add New' button.
2. Type in Readline in the search form and press the 'Enter' key on your keyboard.
3. Click on the 'Activate' button to use your new theme right away.
4. Navigate to Appearance > Customize in your admin panel and customize to taste.

== Documentation ==

http://alx.media/documentation/

== Copyright ==

Readline WordPress Theme, Copyright 2021 AlxMedia
Readline is distributed under the terms of the GNU GPL

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 2 of the License, or
(at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU General Public License for more details.

Readline bundles the following third-party resources:

Font Awesome
Copyright (c) Dave Gandy
License: SIL OFL 1.1 (Fonts); MIT (CSS)
Source: https://fontawesome.com/

Titillium Font
Copyright (c) Accademia di Belle Arti di Urbino
License: SIL OFL 1.1
Source: http://www.campivisivi.net/titillium/

Flexslider
Copyright (c) WooThemes
License: GPLv2 or later
Source: http://flexslider.woothemes.com/

OwlCarousel
Copyright (c) Bartosz Wojciechowski
License: MIT
Source: http://www.owlgraphic.com/owlcarousel/

FitVids
Copyright (c) Chris Coyier
License: WTFPL
Source: http://fitvidsjs.com/

Kirki
Copyright (c) David Vongries
License: MIT
Source: https://kirki.org/

Screenshot images
License: CC0 1.0 Universal (CC0 1.0)
Source: https://stocksnap.io

Header images
1. https://stocksnap.io/photo/Y01VDYAX63 - CC0 1.0 Universal (CC0 1.0)
2. https://stocksnap.io/photo/DGYLV0KMUY - CC0 1.0 Universal (CC0 1.0)
3. https://stocksnap.io/photo/UYYDN7X5IS - CC0 1.0 Universal (CC0 1.0)
4. https://stocksnap.io/photo/UEQ0178WUT - CC0 1.0 Universal (CC0 1.0)
5. https://stocksnap.io/photo/TNK87N7464 - CC0 1.0 Universal (CC0 1.0)

Content images
1. https://stocksnap.io/photo/RKS9M8PY0X - CC0 1.0 Universal (CC0 1.0)
2. https://stocksnap.io/photo/X4AAF4SU9Q - CC0 1.0 Universal (CC0 1.0)

== Changelog ==

= 1.2.9 - 2022-05-30 =
* Fixed ol and ul box-sizing content-box styling for WP 6.0
* Updated to Kirki 4.0.24

= 1.2.8 - 2022-03-15 =
* Updated to Kirki 4.0.22

= 1.2.7 - 2021-08-25 =
* Added missing theme tags for wordpress.org

= 1.2.6 - 2021-07-23 =
* Disabled support for widgets block editor
* Updated language files

= 1.2.5 - 2021-07-19 =
* Updated to Kirki 3.1.9

= 1.2.4 - 2021-07-03 =
* Updated to Kirki 3.1.8

= 1.2.3 - 2021-05-25 =
* Fixed compatibility issue with custom nav and various security plugins
* Updated language files

= 1.2.2 - 2021-04-08 =
* Fixed: avatar image and avatar author link

= 1.2.1 - 2021-03-03 =
* Fixed: mobile menu tabbing will enter in the menu even when it's closed
* Fixed: add styles to focus states of menu and search
* Fixed: trap focus in search popup
* Fixed: readme format to declare license information
* Fixed: make sure that the pingback_url is conditional since pings can be turned off
* Fixed: added no minified font awesome css
* Increased menu dropdown width
* Added option to disable custom navigation in child themes
* Updated language files
* Updated to Font Awesome Free 5.15.2

= 1.2.0 - 2020-08-24 =
* Fixed markup of header and footer menus

= 1.1.9 - 2020-08-18 =
* Fixed header search button outline in Chrome

= 1.1.8 - 2020-08-17 =
* Fixed a11y menu dropdown keyboard nav visibility

= 1.1.7 - 2020-08-17 =
* Improved menu

= 1.1.6 - 2020-08-14 =
* Improved a11y
* Added new menu

= 1.1.5 - 2020-08-11 =
* Updated to Kirki 3.1.5

= 1.1.4 - 2020-07-12 =
* Updated to Kirki 3.1.4

= 1.1.3 - 2020-03-25 =
* Removed ?filter=5 from theme review link
* Removed responsive css on/off option
* Removed unused alx-extensions plugin override css
* Improved support for Gutenberg
* Updated to Kirki 3.1.0
* Updated language files

= 1.1.2 - 2020-02-25 =
* Fixed comments form checkbox position
* Updated to Font Awesome 5.12.1
* Updated to Kirki 3.0.45

= 1.1.1 - 2019-08-11 =
* Updated theme description

= 1.1.0 - 2019-07-29 =
* Fixed Font Awesome square size

= 1.0.9 - 2019-07-28 =
* Updated to Font Awesome 5 Free (5.9.0)

= 1.0.8 - 2019-07-26 =
* Removed Responsive Lightbox as recommended plugin
* Removed duplicate code from functions.php
* Updated language files

= 1.0.7 - 2019-07-26 =
* Added theme accessibility with skip links
* Added wp_body_open to header.php
* Fixed WP-PageNavi styling issue
* Fixed themeforms styling issue
* Updated language files

= 1.0.6 - 2019-03-18 =
* Removed unused code

= 1.0.5 - 2019-03-11 =
* Updated to Kirki 3.0.38
* Changed all alxmedia.se links to alx.media

= 1.0.4 - 2019-02-23 =
* Fixed post hover color styling option for related posts

= 1.0.3 - 2019-02-23 =
* Added post hover color styling option
* Added home/archive/search content width styling option
* Added page content width styling option
* Added post content width styling option

= 1.0.2 - 2019-02-22 =
* Updated screenshot

= 1.0.1 - 2019-02-22 =
* Updated language file
* Added customizer link to AlxMedia
* Added customizer link to Theme Reviews
* Added header top color styling option
* Added footer menu color styling option
* Added footer bottom color styling option
* Added comment count color styling option
* Added carousel next/prev color styling option
* Added page title color styling option
* Added option to disable left social/search bar
* Added boxed layout styling option
* Fixed owl carousel to work better for all resolutions
* Fixed duplicate sharrre headers

= 1.0.0 - 2019-02-22 =
* Initial release
